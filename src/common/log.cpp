...
bool Log::load(const QString &name,QString *err_msg)
{
  ...
  log_name=name;
  log_service_name=QString::fromUtf8(loghdr->log_service);
  log_description=QString::fromUtf8(loghdr->log_description);
  log_origin_username=QString::fromUtf8(loghdr->log_origin_username);
  ...
  for(unsigned i=0;i<ll_quan;i++) {
    ...
    back().setGroupName(QString::fromUtf8(ll[i].logline_group_name));
    back().setGroupColor(ll[i].logline_group_color);
    back().setTitle(QString::fromUtf8(ll[i].logline_title));
    back().setArtist(QString::fromUtf8(ll[i].logline_artist));
    back().setAlbum(QString::fromUtf8(ll[i].logline_album));
    back().setYear(ll[i].logline_year);
    back().setLabel(QString::fromUtf8(ll[i].logline_label));
    back().setClient(QString::fromUtf8(ll[i].logline_client));
    back().setAgency(QString::fromUtf8(ll[i].logline_agency));
    back().setPublisher(QString::fromUtf8(ll[i].logline_publisher));
    back().setComposer(QString::fromUtf8(ll[i].logline_composer));
    back().setConductor(QString::fromUtf8(ll[i].logline_conductor));
    back().setUserDefined(QString::fromUtf8(ll[i].logline_user_defined));
    ...
    back().setMarkerComment(QString::fromUtf8(ll[i].logline_marker_comment));
    back().setMarkerLabel(QString::fromUtf8(ll[i].logline_marker_label));
    back().setOriginUser(QString::fromUtf8(ll[i].logline_origin_user));
    ...
    back().setExtEventId(QString::fromUtf8(ll[i].logline_ext_event_id));
    back().setExtAnncType(QString::fromUtf8(ll[i].logline_ext_annc_type));
  }
  ::free(ll);
  *err_msg="OK";
...
bool Log::save(const QString &name,QString *err_msg)
{
  ...
  if((err=cnf->saveLog(&loghdr,ll,size(),name))!=0) {
